...
export const UpdateUser = ({ user, closeUserEditModal, setUser }) => {
  const {
    handleSubmit,
    register,
    formState: { errors },
  ...
  const { updateWalkdownUser } = useWalkdownManagement();

  const handleUpdate = (data) => {
    setIsPending(true);
    updateWalkdownUser(data)
      .then((response) => {
        if (response) {
          reset();
          setIsPending(false);
  ...
  return (
    <form onSubmit={handleSubmit(handleUpdate)}>
      <div className="flex flex-col gap-4 w-full" data-testid="edit-user-modal">
        <h6 className="text-xl font-fira font-bold leading-6">Edit user</h6>
