...
export const NotScheduledWalkdownTagsTable = ({
  selectedTags,
  setSelectedTags,
  notScheduled,
  pending,
  ...
  const [initialAreaAndUnitData, setInitialAreaAndUnitData] = useState({
    areaName: [],
    unitCode: [],
  });
  ...
  const fetchFilteredData = async (column) => {
    ...
    if (specialFields.includes(column)) {
      try {
        ...
        const filteredData =
          column === "equipmentStatusName"
            ? data.filter(({ value }) => !EXCLUDED_STATUSES.includes(value))
            : data;

        setFilterResultOfColumn((prev) => ({
          ...prev,
          [column]: filteredData.reduce((acc, { key, value }) => {
            const trimmedValue =
  ...
  const handleColumnSearchValue = (event, column) => {
    ...
    if (areaAndUnitFields.includes(column)) {
      setFilterResultOfColumn((prev) => ({
        ...prev,
        [column]: searchValue.length
          ? initialAreaAndUnitData[column].filter((item) =>
              item.value.toLowerCase().includes(searchValue.toLowerCase())
            )
          : initialAreaAndUnitData[column],
      }));
    }
  ...
  const RowRenderer = ({ key, index, style }) => {
    ...
    return (
      <ListItem key={key} style={customStyle}>
  ...
  const isFilterApplied = (column) => {
    return column === "piD"
      ? filterByColumnParams[NOT_ASSIGNED_TAGS_FIELDS[column]] === null
      : !filterByColumnParams[NOT_ASSIGNED_TAGS_FIELDS[column]]?.length;
  };

  const isResetBtnEnabled =
    filterByColumnParams[NOT_ASSIGNED_TAGS_FIELDS[filterColumn]]?.length ||
    (filterColumn === "piD" &&
      filterByColumnParams[NOT_ASSIGNED_TAGS_FIELDS[filterColumn]] !== null);
  ...
  return (
    <Paper
      ...
      elevation={0}
      variant="outlined"
      data-testid="walkdown-tags-table-container"
    >
                  ...
                  {col.id === "checkbox" ? (
                    ...
                    <div className="flex items-center">
                      ...
                      <Badge
                        color="error"
                        variant="dot"
                        invisible={isFilterApplied(col.id)}
                      >
                        <IconButton
                          onClick={(e) => handleFilterClick(e, col.id)}
                          data-testid={`filter-button-${col.id}`}
                        >
                        ...
                        {col.id === "checkbox" ? (
                          <TableCell width={col.width}>
                            <Checkbox
                              ...
                              disabled={pending}
                              data-testid={`tag-checkbox-${row.equipmentTag}`}
                            />
                          </TableCell>
                        ) : (
                          <TableCell width={col.width}>
