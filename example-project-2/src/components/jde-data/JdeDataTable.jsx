...
import {
  ...
  getSelectedTags,
} from "../../store/slices/jde-data/selectedTagsSlice";
...
import { JdeDataHeader } from ".";
import { useJdeData } from "../../hooks/useJdeDataHook";

import { TABLE_COLUMN_LABELS } from "../../constants/jde-data";

export const JdeDataTable = ({
  rows,
  columns,
  loading,
  totalRows,
  ...
  const { getFilteredColumnData } = useJdeData();
  ...
  const handleClearSort = () => {
    dispatch(
      setFilterParams({
        ...filterParams,
        orderColumn: "equipmenT_TAG", // default value
  ...
  const handleRowDoubleClick = (row) => {
    handleLinkClick(row);
    navigate(`/jde-data/${row.asseT_NUMBER_PHK}`, {
  ...
  const handleRowsPerPageChange = (event) => {
    ...
    localStorage.setItem("jdeRowsPerPage", rowsPerPage);
  };
    ...
    switch (col.id) {
      ...
      case "equipmenT_TAG":
        return (
          ...
          >
            <Link
              to={`/jde-data/${row.asseT_NUMBER_PHK}`}
              state={{ from: window.location.pathname }}
    ...
    if (label) {
      return (
        <Chip
          ...
          data-testid="selected_category_chip"
        />
      );
    }
  };
    ...
    if (label) {
      return (
        <Chip
          ...
          data-testid="selected_class_chip"
        />
      );
    }
  };
    ...
    if (label) {
      return (
        <Chip
          ...
          data-testid="selected_subclass_chip"
        />
      );
    }
  };
    ...
    if (filterParams.FilteredColums) {
      ...
      return chips.map(({ label, filterValue }) => (
        <Chip
          ...
          data-testid={`filter_chip_${label}`}
        />
      ));
    }
  };
  ...
  return (
    <Box sx={{ width: "100%" }} data-testid="jde_data_table">
      <Paper
