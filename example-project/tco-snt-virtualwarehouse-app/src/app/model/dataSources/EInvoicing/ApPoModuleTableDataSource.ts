...
import { EsfJdeApPoMatchDto, GetJdePoMatchesResponseDto, JdeClient, PagingModel, SortingOrder } from "src/app/api/GCPClient";
...
@Injectable()
export class ApPoModuleTableDataSource extends DataSourceBaseEntity<GetJdePoMatchesResponseDto, JdeClient>{
    ...
    loadSubjects(): Observable<GetJdePoMatchesResponseDto[]> {
        return this.apiClient.getJdeApPoMatches(
            null,
            null,
            null,
            null,
    ...
    loadApPoWithFilters(filter: FilterModel<ApReconciliationStatusesEnum>, pageIndex: number = 1, pageSize: number = 1000): Observable<EsfJdeApPoMatchDto[]> {
        return this.apiClient.getJdeApPoMatches(
            filter?.dateFrom,
            filter?.dateTo,
            filter?.number,
