...
@Component({
    ...
    providers: [
        ApSoModuleTableDataSource,
        {
            provide: InvoiceCurrentModuleToken,
            useValue: InvoiceModuleMode.ReconciliationContract
        },
        {
...
export class SoComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    public dataSource: ApSoModuleTableDataSource,
    ...
    this.manualReconService.apMatchType = ApMatchType.ApSoMatch
  }

  ngOnInit(): void {

    this.dataSource.apiClient.getSuppliersForApSo()
    .pipe(
  ...
  ngAfterViewInit(){
    this.paginator.page
    .pipe(
      tap(() => this.loadApSo(this.filter, this.paginator.pageIndex + 1, this.paginator.pageSize)),
      takeUntil(this.dataSource.subscription$)
    )
    .subscribe();
  }
  ...
  onFilterOutputChange(filter: FilterModel<ApReconciliationStatusesEnum | ArReconciliationStatusesEnum>) {
    ...
    this.paginator === undefined ? this.loadApSo(this.filter, 1, this.pageSize) 
      : this.loadApSo(this.filter, this.paginator.pageIndex + 1, this.paginator.pageSize);

  }

  private loadApSo(filter: FilterModel<ApReconciliationStatusesEnum>, pageIndex: number, pageSize: number) {
    this.dataSource.loadingSubject.next(true);

    this.dataSource.loadApSoWithFilters(filter, pageIndex, pageSize)
    .pipe(
